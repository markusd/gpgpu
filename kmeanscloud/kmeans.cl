...
float distsqr____(__global float* a, __global float* b)
{
	float result = 0.0f;
...
__kernel void compute_cost(__global float* input, __global float* centroids, __global int* mapping, __global float* reduction)
{
	int id = get_global_id(0);
	reduction[id] = distsqr____(&input[id*DIM], &centroids[mapping[id]*DIM]);
}
