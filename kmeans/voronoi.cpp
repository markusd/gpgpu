...
bool Voronoi::generateVoronoi(float *xValues, float *yValues, int numPoints, float minX, float maxX, float minY, float maxY, float minDist)
{
	cleanup();
	cleanupEdges();
	int i;
...
struct Halfedge* Voronoi::HEcreate(struct Edge *e,int pm)
{
...
void Voronoi::ELinsert(struct	Halfedge *lb, struct Halfedge *newHe)
{
	newHe -> ELleft = lb;
...
struct Halfedge * Voronoi::ELleftbnd(struct Point *p)
{
	int i, bucket;
	struct Halfedge *he;
...
void Voronoi::ELdelete(struct Halfedge *he)
{
...
struct Halfedge * Voronoi::ELright(struct Halfedge *he)
{
	return (he -> ELright);
}

struct Halfedge * Voronoi::ELleft(struct Halfedge *he)
{
	return (he -> ELleft);
}


struct Site * Voronoi::leftreg(struct Halfedge *he)
{
...
struct Site * Voronoi::rightreg(struct Halfedge *he)
{
...
struct Edge * Voronoi::bisect(struct Site *s1,struct	Site *s2)
{
	float dx,dy,adx,ady;
	struct Edge *newedge;	
...
struct Site * Voronoi::intersect(struct Halfedge *el1, struct Halfedge *el2, struct Point *p)
{
	struct	Edge *e1,*e2, *e;
	struct  Halfedge *el;
	float d, xint, yint;
...
int Voronoi::right_of(struct Halfedge *el,struct Point *p)
{
	struct Edge *e;
	struct Site *topsite;
...
void Voronoi::endpoint(struct Edge *e,int lr,struct Site * s)
{
	e -> ep[lr] = s;
	ref(s);
...
float Voronoi::dist(struct Site *s,struct Site *t)
{
	float dx,dy;
...
void Voronoi::makevertex(struct Site *v)
{
	v -> sitenbr = nvertices;
	nvertices += 1;
	out_vertex(v);
...
void Voronoi::deref(struct Site *v)
{
	v -> refcnt -= 1;
	if (v -> refcnt == 0 ) 
...
void Voronoi::ref(struct Site *v)
{
	v -> refcnt += 1;
}

//push the HalfEdge into the ordered linked list of vertices
void Voronoi::PQinsert(struct Halfedge *he,struct Site * v, float offset)
{
...
void Voronoi::PQdelete(struct Halfedge *he)
{
...
int Voronoi::PQbucket(struct Halfedge *he)
{
	int bucket;
...
struct Halfedge * Voronoi::PQextractmin()
{
...
void Voronoi::freeinit(struct Freelist *fl,int size)
{
	fl -> head = (struct Freenode *) NULL;
	fl -> nodesize = size;
}

char * Voronoi::getfree(struct Freelist *fl)
{
	int i; 
	struct Freenode *t;
...
void Voronoi::makefree(struct Freenode *curr,struct Freelist *fl)
{
...
void Voronoi::pushGraphEdge(float x1, float y1, float x2, float y2)
{
...
void Voronoi::out_bisector(struct Edge *e)
{
...
void Voronoi::out_ep(struct Edge *e)
{
	
	
}

void Voronoi::out_vertex(struct Site *v)
{
	
}


void Voronoi::out_site(struct Site *s)
{
...
void Voronoi::out_triple(struct Site *s1, struct Site *s2,struct Site * s3)
{
...
void Voronoi::clip_line(struct Edge *e)
{
	struct Site *s1, *s2;
